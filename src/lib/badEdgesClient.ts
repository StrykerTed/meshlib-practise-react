...
export interface BadEdgesDetectResult {
  badEdgesCount: number;
  badContoursCount: number;
  boundaryEdgesCount: number;
  nonManifoldEdgesCount: number;
...
export class BadEdgesClient {
  ...
  private createWorker() {
    return new Worker(new URL('../workers/badEdges.worker.ts', import.meta.url), {
      type: 'module',
    });
  }
  ...
  private waitForReady(worker: Worker) {
    return new Promise<void>((resolve, reject) => {
      ...
      const onError = (e: ErrorEvent) => {
        cleanup();
        reject(new Error(`BadEdges worker failed to start: ${e.message || 'unknown error'}`));
      };
      const cleanup = () => {
      ...
      const t = setTimeout(() => {
        cleanup();
        reject(new Error('BadEdges worker did not become ready (timeout)'));
      }, 15_000);
    });
  }
  ...
  async detect(
    ...
    return new Promise((resolve, reject) => {
      ...
      const onMessage = (e: MessageEvent<WorkerMessage>) => {
        ...
        resolve({
          badEdgesCount: resp.badEdgesCount,
          badContoursCount: resp.badContoursCount,
          boundaryEdgesCount: resp.boundaryEdgesCount,
          nonManifoldEdgesCount: resp.nonManifoldEdgesCount,
          manifoldEdgesCount: resp.manifoldEdgesCount,
          orientationInvalidTwoNeighborEdgesCount:
            resp.orientationInvalidTwoNeighborEdgesCount,
        });
      };
      ...
      const onError = (e: ErrorEvent) => {
        ...
        cleanup();
        this.resetWorker();
        reject(new Error(`BadEdges worker error: ${e.message || 'unknown error'}`));
      };
      const onMessageError = () => {
        ...
        cleanup();
        this.resetWorker();
        reject(new Error('BadEdges worker messageerror'));
      };
