...
export class FillHolesClient {
  ...
  private createWorker() {
    return new Worker(
      new URL("../workers/fillHoles.worker.ts", import.meta.url),
      {
        type: "module",
      },
    );
  ...
  private waitForReady(worker: Worker) {
    return new Promise<void>((resolve, reject) => {
      ...
      const onError = (e: ErrorEvent) => {
        ...
        reject(
          new Error(
            `FillHoles worker failed to start: ${e.message || "unknown error"}`,
          ),
        );
      };
      const cleanup = () => {
  ...
  async fillHoles(
  ...
  ): Promise<{ output: ArrayBuffer; holesFilledCount?: number }> {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const onMessage = (e: MessageEvent<WorkerMessage>) => {
        ...
        console.log("[FillHoles Client] Response received:", {
          outputSize: resp.output.byteLength,
          holesFilledCount: resp.holesFilledCount,
        });
      ...
      const onError = (e: ErrorEvent) => {
        ...
        this.resetWorker();
        reject(
          new Error(`FillHoles worker error: ${e.message || "unknown error"}`),
        );
      };
      ...
      const onMessageError = () => {
        ...
        reject(
          new Error(
            "FillHoles worker messageerror (could not deserialize message)",
          ),
        );
      };
