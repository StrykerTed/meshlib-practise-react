...
export class AnnotationsClient {
    ...
    async selectPatch(
        ...
        const raw = await this.send('select_patch', {
            stlBuffer,
    ...
    async createLandmark(
        ...
        return this.send('create_landmark', {
            stlBuffer,
    ...
    async patchFromLandmarks(
        ...
        const raw = await this.send('patch_from_landmarks', {
            stlBuffer,
    ...
    private send(type: string, payload: any, onStatus?: (s: string) => void): Promise<any> {
